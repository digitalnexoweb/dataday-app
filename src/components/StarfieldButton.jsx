...
// Adaptado para DataDay:
//   - Convertido de .tsx a .jsx (el proyecto no usa TypeScript).
//   - Se agregaron las props `onClick` y `disabled`, que el original no
//     reenviaba al <button>. Marcadas con [DATADAY] mas abajo.
// El resto del componente es identico al de Originkit. No usa Tailwind:
 ...
 *  so every side is read on its own. */

/** Hard ceiling on band width. */
...
/** Acepta `#rgb`, `#rrggbb`, `#rrggbbaa`, `rgb()`, `rgba()` o `var(--token, <value>)`. */
...
/** Rounded es un porcentaje del radio maximo posible (la mitad del lado corto),
 *  asi 100 es una pastilla real en cualquier tamano. */
const radiusFromPercent = (w, h, pct) =>
    (Math.min(w, h) / 2) * (Math.max(0, Math.min(100, pct)) / 100)

/** La banda del borde: la caja redondeada completa menos la caja de contenido,
 *  compuesta con `exclude`. Las luces viajeras viven adentro, sobre el trazo. */
const BAND_MASK = {
...
// El layout debe resolverse antes del primer pintado, si no el boton aparece con
// esquina cuadrada durante un frame.
const useIsoLayoutEffect =
    typeof window !== "undefined" ? useLayoutEffect : useEffect

// Semilla determinista (nada de Math.random: desincronizaria entre renders).
const rnd = (i, salt) => {
    const x = Math.sin((i + 1) * 12.9898 + salt * 78.233) * 43758.5453
    return x - Math.floor(x)
}

/** Punto a la fraccion `t` del perimetro de un rect redondeado, medido por
 *  LONGITUD DE ARCO: velocidad constante y siempre sobre el borde. */
...
/** Punto donde un rayo desde el CENTRO en la fraccion de angulo `t` cruza el
 *  mismo contorno: velocidad ANGULAR constante (modo "step"). */
...
// Segundos por vuelta a una unidad de la escala interna de velocidad.
...
function OriginkitBaseStarfieldButton(props) {
    const {
        label = "STARFIELD",
        font = {
            variant: "Regular",
        ...
        newTab = false,
        style,
        // [DATADAY] Estas dos no existian en el original: el componente nunca
        // reenviaba onClick ni disabled al <button>, asi que un boton real era
        // inerte. Se agregan aca para conservar la accion y la proteccion del
        // boton que reemplaza.
        onClick,
        disabled = false,
    } = props
    ...
    const Tag = link && !disabled ? "a" : "button"
    const tagProps = {
        ...
        ...(link && !disabled
            ...
            : // [DATADAY] onClick y disabled reenviados al <button> real, para
              // que el click dispare la misma accion y el estado deshabilitado
              // se respete de verdad (no solo visualmente).
    ...
    // [DATADAY] Deshabilitado: sin animacion de hover ni de presion.
    const onEnter = () => {
        if (disabled) return
        animateReveal(1)
    }
        ...
        iconMode === "image" ? (
            <img
                ...
                style={{
                    ...
                    borderRadius: radiusFromPercent(
                        iconPx,
                        iconPx,
                        iconRounded
                    ),
        ...
        >
            <Tag
                ...
                style={{
                    ...
                    // [DATADAY] mismo tratamiento visual que `.primary-button:disabled`
